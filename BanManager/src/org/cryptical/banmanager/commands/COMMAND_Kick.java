...
import org.cryptical.banmanager.cevents.PlayerKickEvent;
...
public class COMMAND_Kick implements CommandExecutor, TabCompleter{
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args.length > 1) {
			...
			Punisher.kick(target, reason);
			return true;
		}
		if (args.length == 1) Punisher.kick(target);

		Bukkit.getPluginManager().callEvent(new PlayerKickEvent(target));
		Bukkit.getPluginManager().callEvent(new PlayerPunishedEvent(target, PunishReason.KICK));
		return true;
	}

	public List<String> onTabComplete(CommandSender sender, Command cmd, String label, String[] args) {
		List<String> matches = new ArrayList<>();
		for (CPlayer p : Core.playerData.getOnlinePlayers()) {
