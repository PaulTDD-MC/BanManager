...
public class COMMAND_Mute implements CommandExecutor, TabCompleter{
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args.length > 1) {
			...
			Punisher.mute(target, reason);
			return true;
		}
		if (args.length == 1) Punisher.mute(target);
		
		Bukkit.getPluginManager().callEvent(new PlayerMuteEvent(target));
		Bukkit.getPluginManager().callEvent(new PlayerPunishedEvent(target, PunishReason.MUTE));
		return true;
	}
