...
public class COMMAND_Ban implements CommandExecutor, TabCompleter{
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args.length > 1) {
			...
			Punisher.ban(target, reason);
		}
		if (args.length == 1) Punisher.ban(target);

		Bukkit.getPluginManager().callEvent(new PlayerBanEvent(target));
		Bukkit.getPluginManager().callEvent(new PlayerPunishedEvent(target, PunishReason.BAN));
