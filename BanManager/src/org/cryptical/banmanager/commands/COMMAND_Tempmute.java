...
import org.cryptical.banmanager.cevents.PlayerMuteEvent;
...
public class COMMAND_Tempmute implements CommandExecutor, TabCompleter{
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args.length > 2) {
			...
			Punisher.tempmute(target, till, reason);
			return true;
		}
		if (args.length == 1) Punisher.tempmute(target, till);

		Bukkit.getPluginManager().callEvent(new PlayerMuteEvent(target));
		Bukkit.getPluginManager().callEvent(new PlayerPunishedEvent(target, PunishReason.TEMPMUTE));
		return true;
	}
